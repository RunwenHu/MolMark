...
class MolGenValidationCallback(Callback):
    def __init__(self, dataset, atom_type_one_hot=True, single_bond=False, batch_size=64) -> None:
        super().__init__()
    ...
    def on_validation_batch_end(
        ...
        outputs_list = []
        dataval = outputs[0]
        numbatch = dataval.zx.shape[0] // self.batch_size
        for i in range(self.batch_size):
            data = Data(zpos=dataval.zpos[i*numbatch:(i+1)*numbatch],
                        zcharges=dataval.zcharges[i*numbatch:(i+1)*numbatch],
                        num_nodes=numbatch,
                        batch=i,
                        pos=dataval.pos[i*numbatch:(i+1)*numbatch],
                        zx=dataval.zx[i*numbatch:(i+1)*numbatch],
                        edge_index=dataval.edge_index[i*numbatch:(i+1)*numbatch],
                        x=dataval.x[i*numbatch:(i+1)*numbatch],
                        recovery=dataval.recovery[i])
            outputs_list.append(data)

        self.outputs.extend(outputs_list)
...
class MolVisualizationCallback(Callback):
    ...
    def __init__(self, atomic_nb, remove_h, atom_decoder, generated_mol_dir, batch_size=64) -> None:
        super().__init__()
        self.outputs = []
        self.batch_size = batch_size
    ...
    def on_validation_batch_end(
        ...
        if len(self.chain_outputs) == 0:
            ...
            for i in range(len(theta_chain)):
                ...
                outputs_list = []
                dataval = out_data_list[0]
                numbatch = dataval.zx.shape[0] // self.batch_size
                for j in range(self.batch_size):
                    data = Data(zpos=dataval.zpos[j*numbatch:(j+1)*numbatch],
                                zcharges=dataval.zcharges[j*numbatch:(j+1)*numbatch],
                                num_nodes=numbatch,
                                batch=j,
                                pos=dataval.pos[j*numbatch:(j+1)*numbatch],
                                zx=dataval.zx[j*numbatch:(j+1)*numbatch],
                                edge_index=dataval.edge_index[j*numbatch:(j+1)*numbatch],
                                x=dataval.x[j*numbatch:(j+1)*numbatch],
                                recovery=dataval.recovery[j])
                    outputs_list.append(data)
