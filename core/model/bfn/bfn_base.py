...
class bfn4MolEGNN(bfnBase):
    def __init__(
        ...
        self.centers = torch.linspace(
            start, end, self.in_node_nf, device="cuda" if torch.cuda.is_available() else "cpu"
        )  # [feature_num]
        # print(in_node_nf + int(condition_time),in_node_nf + int(condition_time)+1)
        self.K_c = torch.tensor(k_c).to(self.device)
        self.fn = nn.MSELoss()

        self.gamma_coord_list = self.build_gamma_coord()


    def build_gamma_coord(self):
        gamma_coord_list = []
        for i in range(1, self.sample_steps + 1):
            t = torch.ones((1, 1)).to(self.device) * (i - 1) / self.sample_steps
            t = torch.clamp(t, min=self.t_min)
            gamma = 1 - torch.pow(self.sigma1_coord, 2 * t)
            gamma_coord_list.append(torch.sqrt((1 - gamma) / gamma + 1e-8))
        return gamma_coord_list
    ...
    def interdependency_modeling(
        ...
        coord_pred = (mu_pos_t / gamma_coord - torch.sqrt((1 - gamma_coord) / gamma_coord) * eps_coord_pred)
        

        if self.charge_discretised_loss:
            
            sigma_charge_eps = torch.exp(sigma_charge_eps)
            mu_charge_x = (mu_charge_t / gamma_charge - torch.sqrt((1 - gamma_charge) / gamma_charge) * mu_charge_eps)
            sigma_charge_x = (torch.sqrt((1 - gamma_charge) / gamma_charge) * sigma_charge_eps)
    ...
    def loss_one_step(
        ...
        if self.charge_discretised_loss:
            ...
            # k_c = self.K_c.unsqueeze(-1).unsqueeze(0)
            # k_hat = (k_hat * k_c).sum(dim=1, keepdim=True)
            # e_k_c = self.K_c[(k_hat - k_c).abs().argmin(dim=1)]
            # charge_loss = self.ctime4discreteised_loss(
            #     t=t, sigma1=self.sigma1_charges, x_pred=k_hat, x=charges
            # )
        else:
    ...
    def predict_xn(self, x, segment_ids=None):
        gamma_1 = 0
        for i in range(self.sample_steps - 1):
            eps_coord_pred = (torch.randn_like(x) - torch.randn_like(x)) * 1e-2
            # eps_coord_pred = self.zero_center_of_mass(eps_coord_pred, segment_ids)
            eps_coord_pred = torch.clamp(eps_coord_pred, min=-10, max=10)
            gamma_1 += self.gamma_coord_list[i] * eps_coord_pred
            gamma_1 = torch.clamp(gamma_1, min=-10, max=10)
        xn = x +  gamma_1
        return xn
